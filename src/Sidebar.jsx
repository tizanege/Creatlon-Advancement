...
import { Shirt, Layers, Upload, Type, Image as ImageIcon, PaintBucket, Search } from 'lucide-react';
    ...
    <div style={{ display: 'flex', height: '100%' }}>
        ...
        {menuItems.map((item) => (
          <div
            ...
            style={{
              ...
              border:
                active === item.id ? '2px solid var(--primary-color)' : '2px solid transparent',
              background: active === item.id ? 'rgba(251, 133, 70, 0.06)' : 'transparent',
              transition: 'all 0.2s',
            }}
          >
            <item.icon size={22} strokeWidth={active === item.id ? 2.5 : 2} />
            <span style={{ fontSize: '11px', fontWeight: active === item.id ? 600 : 500 }}>
              {item.label}
            </span>
          </div>
        ))}
      ...
      {active === 'file' ? (
        ...
        <div
          ...
          style={{ width: '280px', padding: '32px 24px', alignItems: 'center', zIndex: 5 }}
        >
          <div
            style={{
              width: '160px',
